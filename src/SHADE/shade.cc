...
SHADE::SHADE(CEvaluation<CRealCoding> *evaluation)
	: searchAlgorithm(evaluation)
{
	bsf_solution = nullptr;
...
void SHADE::initialize()
{
	...
	fitness.resize(pop_size, 0);
	children_fitness.resize(pop_size, 0);
	...
	bsf_solution = (variable*)malloc(sizeof(variable) * problem_size);
	...
	memory_sf.resize(memory_size, 0.5);
	memory_cr.resize(memory_size, 0.5);

	memory_pos = 0;

	//for new parameters sampling
	pop_sf = (variable*)malloc(sizeof(variable) * pop_size);
	pop_cr = (variable*)malloc(sizeof(variable) * pop_size);

	//for current-to-pbest/1
	p_num = round(pop_size *  p_best_rate);
	sorted_array = (int*)malloc(sizeof(int) * pop_size);
	temp_fit = (Fitness*)malloc(sizeof(Fitness) * pop_size);
}

void SHADE::runIteration(vector<CGenePattern*> *patterns)
{
	if (patterns == nullptr) {
		...
		for (int target = 0; target < pop_size; target++) {
			...
			operateCurrentToPBest1BinWithArchive(pop, &children[target][0], target, p_best_ind, pop_sf[target], pop_cr[target], archive, arc_ind_count, nullptr);
		}
	  ...
	  //update the bsf-solution and check the current number of fitness evaluations
		...
		for (size_t p = 0; p < patterns->size(); p++) {
			CGenePattern *pattern = patterns->at(p);

			if (pattern->iGetSize() > 3)
			{
				break;
			}
			...
			for (int target = 0; target < pop_size; target++) {
				...
				//p-best individual is randomly selected from the top pop_size *  p_i members
				p_best_ind = sorted_array[rand() % p_num];
				//pattern = patterns->at(rand() % patterns->size());
				operateCurrentToPBest1BinWithArchive(pop, &children[target][0], target, p_best_ind, pop_sf[target], pop_cr[target], archive, arc_ind_count, pattern);
			}
...
void SHADE::operateCurrentToPBest1BinWithArchive(const vector<Individual> &pop, Individual child, int &target, int &p_best_individual, variable &scaling_factor, variable &cross_rate, const vector<Individual> &archive, int &arc_ind_count, CGenePattern *pattern) {
  if (pattern == nullptr) {
	int r1, r2;
	...
	if (r2 >= pop_size) {
		...
		for (int j = 0; j < pattern->iGetSize(); j++) {
			int i = *(pattern->piGetPattern() + j);
			child[i] = pop[target][i] + scaling_factor * (pop[p_best_individual][i] - pop[target][i]) + scaling_factor * (pop[r1][i] - archive[r2][i]);
		}
	}
	else {
